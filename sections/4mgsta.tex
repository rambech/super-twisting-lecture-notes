...
where $J(x)$ is the Jacobian matrix of $\phi_1(x)$ and $c(x)$ is a scalar function, both of which are defined in $\R^n \backslash \left\{0\right\}$, i.e.
\begin{equation}
    J(x) = \frac{\partial\phi_1(x)}{\partial x} = \left(\alpha\norm{x}^{-p}+\beta\right)I - \alpha p \frac{\norm{x}^{-p}}{\norm{x}}xx^\top
\end{equation}
\begin{equation}
    c(x) = \alpha(1 - p)\norm{x}^{-p} + \beta
\end{equation}

Scalar gain $k_1$ and $k_2$, while $b, \alpha, \beta >0$ and $p \in \left(0, \frac{1}{2}\right]$ are arbitrary. 
...
\end{assumption}
\begin{rmk}
    For a matrix $M \in \R^{n \times n}$, $\text{Sym}\left\{M\right\} = \frac{1}{2}(M + M^\top)$, $\text{Gram}\left\{M\right\} = M^\top M$
\end{rmk}
...
\end{equation} 
is satisfied. Then, for arbitrary values of $\delta_2, \delta_3, \delta_3$, there exist positive constant controller gains $k_1, k_2$ such that $\left[x^\top, z^\top\right]^\top$ is globally robust finite-stable.
\end{theorem}
