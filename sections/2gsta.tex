...
\subsubsection{Velocity-tracking problem}
    ...
    \begin{equation}
        \dot{e}_1 = -k_1 \gamma(q) \lfloor e_1 \rceil^{1/2} + \gamma(q)
        \underbrace{
            \left[z - g\sin(q) + b\left(e_1 + \dot{q}_d + \arctan(e_1 + \dot{q}_d) - \frac{\ddot{q}}{\gamma(q)}\right)\right]
            }_{e_2}
    \end{equation}
    Now we differentiate $e_2$ with respect to time
    \begin{equation}
        \dot{e}_2 = -k_2\lfloor e_1 \rceil^0 +
        \underbrace{
            g\cos(q) - \left[\frac{\ddot{q}_d}{\gamma(q)} - \frac{\ddot{q}_d}{\gamma^2(q)}\right] + bc\ddot{q}_d
        }_{\dot{\varphi}}
        + bc\dot{e}_1
    \end{equation}
    where 
    \begin{equation*}
        c = \frac{(e_1 + \dot{q}_d)^2 + 2}{(e_1 + \dot{q}_d)^2 + 1}
    \end{equation*}
    \begin{equation}
        \begin{split}
    ...
    \end{equation}
    \begin{enumerate}
        \item STA gain $k_2$ needs to be design based on a duncton depengind on STA gains $k_1$ and $k_2$. This is an algebraic loop.
        \item Although the perturbations $\dot{\varphi}$ can be bounded by a constant, the terms $- bc\gamma k_1 \lfloor e_1 \rceil^{1/2} - bc\gamma k_2 \int_{0}^{t} \lfloor e_1(\tau) \rceil^0 d\tau$ can be very large depending on initial conditions $e_1(0)$ and perturbations.
        \item The time variation in the control coefficient $\gamma(q(t))$ introduces the term $\frac{\ddot{q}_d}{\gamma^2(q(t))}\dot{\gamma}(q(t))$ to the total time-derivative, and the bounds of $\gamma(q(t))$ and its derivative $\dot{\gamma}(q(t))$ should be also taken into account in the controller gains design.
        \item State-dependent perturbation imply state-dependent derivative which make the perturbation grow exponentially in time.
    \end{enumerate}
...
Closed-loop system
      ...
      \begin{rmk}
        In the paper, the state $\sigma_2$ is referred to as a dynamic extension
      \end{rmk}
   ...
   \begin{theorem}[Castillo et al. \cite{Cas:18}]
      ...
      \begin{equation}
         T(\sigma(0)) = \frac{2}{\mu_2} \ln \left(\frac{\mu_2}{\mu_1}V^{1/2}(\sigma_1(0), \sigma_2(0)) + 1\right)
      \end{equation}
   \end{theorem}

Choosing gains (Figures here)
%    \begin{figure}[h]
...
%    \end{figure}
\begin{rmk}
    The GSTA gain $\beta$ does not only allow to guarantee the global stability of the system. It can also help reduce the chattering effect by reducing the minimum value of gains $k_1$ and $k_2$. Figure
\end{rmk}
...
\begin{equation*}
    \left(1 + \cos^2(q)\right)\ddot{q} + g\sin(q) + b\left(\dot{q} + \arctan (\dot{q})\right) = u
\end{equation*}
The error dynamics, with error state $e_1 = \dot{q} - \dot{q}_d$, in closed-loop (probably already on the board)
\begin{equation}
    \dot{e}_1 = \gamma(q)u - \gamma(q)g\sin(q) - \gamma(q)b(e_1 + \dot{q}_d) + \arctan(e_1 + \dot{q}) - \ddot{q}
\end{equation}
With GSTA, and perturbations split into two parts is 
\begin{equation}
    \dot{e}_1 = -k_1\gamma(q)\phi_1(e_1)
    \underbrace{
        - \gamma(q)be_1
    }_{\varphi_1(e_1, t)} 
    + \gamma(q)
    \left[z
    \underbrace{
        - g\sin(q) - b\left(\dot{q}_d + \arctan(e_1 + \dot{q}_d)\right) - \frac{\ddot{q}_d}{\gamma(q)}
    }_{\frac{\varphi_2(e_1, t)}{\gamma(q)}}
    \right]
\end{equation}

Then the system with dynamic extension (what does he mean with dynamic extension) $e_2 = z + \frac{\varphi_2(e_1, t)}{\gamma(q)}$ can be written on the form

\begin{equation}
    \begin{split}
        \dot{e}_1 &= -\gamma(q)k_1\phi_1(e_1) + \gamma(q)e_2 + \varphi_1(e_1, t) \\
        \dot{e}_2 &= -k_2\phi_2(e_1) + \frac{d}{dt}\left(\frac{\varphi_2(e_1, t)}{\gamma(q)}\right)
    \end{split}
\end{equation}
% where 
% \begin{equation*}
%     \frac{\varphi_2(e_1, t)}{\gamma(q)} = - g\sin(q) - b\left(\dot{q}_d + \arctan(e_1 + \dot{q}_d)\right) - \frac{\ddot{q}_d}{\gamma(q)}
% \end{equation*} 

With this design, the problem of the algebraic loop, uncertain control coefficient and state-dependent perturbations can be solved.
