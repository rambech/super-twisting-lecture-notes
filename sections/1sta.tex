...
To simplify the Lyapunov analysis we perform a change of variable
    ...
    Equation \eqref{eq:shtessel-transformed-system} can then be written on matrix form
        ...
        \begin{equation}
          V(z_1, z_2, \alpha, \beta) = V_0 + \frac{1}{2\gamma_1}\left(\alpha - \alpha^*\right)^2 + \frac{1}{2\gamma_2}\left(\beta - \beta^*\right)^2
        \end{equation}
        where 
        \begin{equation}
          V_0(z) = \left(\lambda+4\epsilon^2\right)z_1^2 + z_2^2 - 4\epsilon z_1 z_2 = z^\top P z
        \end{equation}
        \begin{equation*}
          P = 
          \begin{bmatrix}
       ...
       To ensure positive definite $\tilde{Q}$ we enforce
        ...
        We also have that  
        \begin{equation}
          \lambda_{\text{min}}\left(P\right)\norm{z}^2 \leq z^\top P z \leq \lambda_{\text{max}}\left(P\right)\norm{z}^2 
        \end{equation}
        ...
        \textbf{Observe.}
        For the controller to converge in finite time the inequality \eqref{eq:shtessel-inequality-19}. This implies that $\alpha(t)$ must increase according to \eqref{eq:shtessel-alpha-dot} until \eqref{eq:shtessel-inequality-19} is met in order to guarantee a positive definite $\tilde{Q}$ matrix.
